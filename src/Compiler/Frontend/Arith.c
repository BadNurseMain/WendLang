...
void writeString(int8_t* String)
{
	//Writing the Size and String.
	uint16_t Size = (uint16_t)strlen(String);
	fwrite(&Size, 1, 2, IntermediateFile);
...
uint32_t writeArithmeticOperations(uint32_t StartLocation, LocalNameStruct* Variables, uint32_t VariableCount, uint8_t OptionalParam)
{
    uint8_t* String = 0;
    ...
    uint16_t OriginalEquationLength = 0;
    int8_t* OriginalEquationString = malloc(512);

    //For writing out Original Equation.
    if (TokenBuffer[StartLocation - 2][0] == ':')
    {
        String = stringifyInstruction(5, "; ", TokenBuffer[StartLocation - 3], " : ", TokenBuffer[StartLocation - 1], " {\0");
        OriginalEquationLength = strlen(String);

        strncpy(OriginalEquationString, String, 512);
        //fwrite(String, 1, strlen(String), IntermediateFile);
        free(String);
    }
    else
    ...
    do
    {
        ...
        //For Original Equation.
        OriginalEquationLength += strlen(TokenBuffer[MaxCount]);
        strcat(OriginalEquationString, TokenBuffer[MaxCount]);

        MaxCount++;
    } while (TokenBuffer[MaxCount][0] != ';');

    OriginalEquationLength++;
    strcat(OriginalEquationString, "}\n\0");

    writeString(OriginalEquationString);
    free(OriginalEquationString);
...
uint32_t writeConditionalOperations(uint8_t* FunctionName, uint32_t StartLocation, LocalNameStruct* Variables, uint32_t* VariableCount, uint32_t* ConditionalCount, uint8_t OptionalParam)
{
    ...
    //Getting the Equation Size.
    uint16_t OriginalSize = 3;
    for (uint32_t x = StartLocation - 1; x < Loop; x++)
    	OriginalSize += (uint16_t)strlen(TokenBuffer[x]);

    fwrite(&OriginalSize, 1, 2, IntermediateFile);
    ...
    notComplexCompare:
        ...
        if (Value == LessThan)
        {
            String = stringifyInstruction(3, INSTRUCTIONS[GreaterThan], JumpName, "\n\0");
        ...
        {
            String = stringifyInstruction(3, INSTRUCTIONS[LessThan], JumpName, "\n\0");
        ...
        if(Value == Equality)
        {
            String = stringifyInstruction(3, INSTRUCTIONS[UnEquality], JumpName, "\n\0");
